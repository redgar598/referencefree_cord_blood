...
## load summary table made in excel
meta_summary<-read.csv("~/ewas3rdround/summarytable.csv", sep=";")
meta_summary$X.1<-NULL

colnames(meta_summary)<-c("","UBC (n=24)", "GenR (n=191)")
...
\newpage
```{r, echo=FALSE,  message=FALSE}
methods_table<-read.csv("~/ewas3rdround/methods_table.csv")
...
load("~/ewas3rdround/R2_plot.RData")
levels(plt_r2$facs_celltype)<-c("NK Cells", "B Cells", "CD8 T Cells","CD4 T Cells","Granulocytes", "Monocytes")
...
ggsave("~/ewas3rdround/figures/Figure1R2.pdf", width = 9, height = 5, units = "in")
```
...
## mean boxplot
ggplot(plt_r2, aes(reorder(method, R2_LRT, FUN=median), R2_LRT))+geom_boxplot(outlier.shape=NA, fill="grey95")+
...
ggsave("~/ewas3rdround/figures/Figure2R2.pdf", width = 8, height = 7, units = "in")


```

```{r R23, fig.width=8,  fig.height=5, echo=FALSE,  message=FALSE,fig.cap='Variance not explained in each method component by FACS cell counts.Models used FACS cells counts to model components from several cell type estimation methods.'}
load("~/ewas3rdround/R2_inverse_plot.RData" )

ggplot(R2_inverse_all, aes(reorder(Method, R2_inverse, FUN=median), R2_inverse, fill=Method))+geom_boxplot(outlier.shape=NA, fill="grey95")+
...
ggsave("~/ewas3rdround/figures/Figure3R2.pdf", width = 8, height = 7, units = "in")
...
load("~/ewas3rdround/error_all.RData")

error_all_unique<-error_all
#error_all_unique<-error_all[!duplicated(error_all$mn_cpg_mae),]
#error_all_unique<-error_all_unique[-1,]

error_all_unique$col_error<-cut(error_all_unique$mn_cpg_mae, c(0,0.001, 0.005,0.0075,0.01,0.05), right=FALSE, labels=c("<0.001","<0.005","<0.0075","<0.01",">0.01"))
...
ggplot(error_all_unique, aes(method_1,method_2, fill = col_error)) +
  ...
  scale_fill_manual(values=c("#084594","#4292c6","#9ecae1","#c5e7f9","#deebf7",""), name="MAE")+th+
  theme(axis.text.y=element_text(size=12,color="black"),
        axis.text.x = element_text(size=12,color="black",angle = 90, hjust = 1),
        panel.border = element_blank(),
        axis.ticks=element_blank())+xlab("")+ylab("")

ggsave("~/ewas3rdround/figures/maehalf.pdf", width = 13, height = 11, units = "in")
...
```{r correlation, fig.width=12,  fig.height=10, echo=FALSE,  message=FALSE, fig.cap='Distributions of correlation of beta values across samples at each CpG. Betas across samples were correlated between each method and the gold-standard FACS-PCA corrected betas. Vertical blue line shows the median correlation across all CpGs for the Deconvolution PCA corrected betas.'}
 load("~/ewas3rdround/correlation_distributions.RData")
...
ggsave("~/ewas3rdround/figures/Figure3cor.pdf", width = 12, height = 10, units = "in")
```
...
ggsave("~/ewas3rdround/figures/Figure4decon.pdf", width = 9, height = 11, units = "in")
...
load("~/ewas3rdround/mads_melted.RData")


mads.melt$col_mad<-cut(mads.melt$value, c(0,0.005, 0.01,0.02,0.1), right=FALSE, labels=c("<0.005","<0.01","<0.02",">0.05"))
...
ggsave("~/ewas3rdround/figures/Figure5decon.pdf", width = 12, height = 10, units = "in")
```

\newpage
```{r, echo=FALSE,  message=FALSE}
sex_table<-read.csv("~/ewas3rdround/Sex_EWAS_hit_tables.csv")
colnames(sex_table)<-c("Method","Hits","True Positives",
                       "False Positives","False Negatives","Spearman","Kendall",
...
levels(sex_table$Method)<-c("Deconvolution - PCA","Deconvolution - Drop One Cell Type",
                            ...
                            "ReFACTor","RefFreeCellMix","RUV","SVA - Supervised",
                            "SVA - Unsupervised","Uncorrected")

sex_table$Spearman<-round(sex_table$Spearman, 3)
#sex_table$Kendall<-round(sex_table$Kendall, 3)
sex_table$Kendall<-NULL



kable(sex_table, format="latex", booktabs = T, caption = "Comparison of cell type correction methods through EWAS results. EWAS was performed for sex on each corrected dataset. True and false positives were calculated in comparison to the sex differentially methylated CpGs in sorted cell type data (n=18).  Spearman is the correlation coefficient for all CpG p values, and Kendall is the rank correlation coefficient for the top 1000 CpG p values.")%>%  kable_styling(font_size = 7,(latex_options = c("HOLD_position")))

```


```{r, echo=FALSE,  message=FALSE}
ga_table<-read.csv("~/ewas3rdround/ga_EWAS_hit_tables.csv")
colnames(ga_table)<-c("Method","Hits","True Positives",
                       "False Positives","False Negatives","Spearman","Kendall",
                       "Sensitivity","Specificity")


levels(ga_table$Method)<-c("Deconvolution - PCA","Deconvolution - Drop One Cell Type",
                            "FACS - PCA - Gold-Standard","FACS - Drop One Cell Type",
                            "ReFACTor","RefFreeCellMix","RUV","SVA - Supervised",
                            "SVA - Unsupervised","Uncorrected")

ga_table$Spearman<-round(ga_table$Spearman, 3)
#ga_table$Kendall<-round(ga_table$Kendall, 3)
ga_table$Kendall<-NULL



kable(ga_table, format="latex", booktabs = T, caption = "Comparison of cell type correction methods through EWAS results. EWAS was performed for GA on each corrected dataset. True and false positives were calculated in comparison to the GA differentially methylated CpGs in sorted cell type data (n=18).  Spearman is the correlation coefficient for all CpG p values, and Kendall is the rank correlation coefficient for the top 1000 CpG p values.")%>%  kable_styling(font_size = 7,(latex_options = c("HOLD_position")))
